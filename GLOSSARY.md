...
**APCA (Accessible Perceptual Contrast Algorithm)** — Perceptual contrast model that more accurately predicts readability than WCAG 2.x's relative-luminance ratio. Candidate for WCAG 3; not yet a compliance standard as of mid-2026. Use as internal target; cite WCAG 2.2 for compliance. The published thresholds live at the **APCA Readability Criterion** (`readtech.org/ARC/`), with a Bronze / Silver / Gold tier framework and a canonical font-size × weight × Lc lookup matrix dated 22 May 2022; the formula is APCA·W3 v0.1.7 and the spec carries a draft / beta status. Asymmetry: identical colour pairs swapped do not yield identical |Lc| because the equation uses different exponents per polarity (light-on-dark vs. dark-on-light), but the threshold table is read by absolute Lc value either way. In token validation pipelines, the discipline is to **block on WCAG failures and warn on APCA failures** — regulatory floor plus readability signal. (See 14-accessibility, 24-tokens-at-scale, 31-color-systems; primary source `_source-text/ext-apca-thresholds.txt`.)
...
**Composite token** — A token holding multiple values as a unit (e.g., a shadow as `[x, y, blur, spread, color]` rather than five separate tokens). Stable in DTCG 2025.10 for typography, shadow, border, and transition. **The 2025.10 spec mandates that property-level references use JSON Pointer (`$ref`), not curly-brace alias syntax** — a composite typography token's `fontFamily` sub-property aliasing an atomic family token must be written `{ "$ref": "#/font-family/text/$value" }`, not `{font-family.text}`; the curly-brace alias works only at the whole-token level. Tools support composites but most still fail to faithfully preserve property-level aliases on round-trip regardless of which syntax the spec mandates. The practitioner discipline: **carry composites as the source-of-truth representation; flatten at the build pipeline into platform-specific atomic outputs** that runtime consumes. (See 22-token-architecture-extensions, 12-figma-practice, 23-typography-tokenisation; primary source `_source-text/ext-dtcg-format-module.txt`.)
