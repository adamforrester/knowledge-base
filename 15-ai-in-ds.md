...
### The design brief as a file — `design.md`

The workflow above has an input the practice has never named: the artefact the AI reads. We have a committed POV on tokens as an interchange format (DTCG — see 22) and on the component data file as a source of truth (see 30), but no POV on the layer *above* tokens — **the design brief as a file that drives generation.** An open standard has now appeared for exactly this: **`design.md`** (`google-labs-code/design.md`, Apache-2.0), "a format spec for describing a visual identity to coding agents." A `design.md` file is two layers in one document — YAML front matter carrying machine-readable tokens (`colors`, `typography`, `spacing`, per-component properties) and a Markdown body carrying human-readable rationale (Overview, Colors, Typography, Layout, Elevation, Shapes, Components, Do's and Don'ts). Humans author it; agents consume it. It is, in other words, the two-audiences principle this whole file rests on (human prose plus machine data, one source) applied to the brief itself — the visual-identity counterpart to the voice-and-tone matrix that already serves as the verbal-identity brief and AI system prompt (see 04).

**Where it sits is the load-bearing point: `design.md` is interchange for the *brief*, DTCG is interchange for the *tokens*.** They compose rather than compete — the design brief is upstream design intent, the DTCG file is the resolved values downstream, and `design.md`'s own tooling exports to DTCG. The pipeline is `design.md (brief) → generate → DTCG (tokens) → Style Dictionary → platforms`. The brief is the source; the token file is a compiled artefact of it — the same source-of-truth shape 30 draws for component data, moved up a layer to the brand brief.

**Our position, measured to the spec's maturity.** The procurement test we apply to DTCG — a stable spec is a commitment you can name in a contract — `design.md` fails on exactly one axis: it is **version `alpha`** as of mid-2026 (verify before relying on it), so it does not yet belong in a client proposal as a named standard the way DTCG 2025.10 does. On every other axis it is safe to lean into: Apache-2.0 removes the legal question, the export-to-DTCG path removes the lock-in question, and its shape matches where our practice already is. **So adopt it now as an internal authoring and interchange convention — the front door to a generative engine — pilot it on a real brand, and track its march to stability, but do not yet sell it as procurement-grade.** It is the first credible open standard for the brief layer, and that is worth saying even while it is alpha. One discipline on adoption: the spec tolerates unknown keys *silently*, which is convenient and dangerous — a key one tool ignores and another treats as load-bearing is a drift vector no validator catches. Namespace any extension explicitly and document its schema, exactly as we do for DTCG `$extensions` (see 22), rather than trusting silent tolerance. Conformance-plus-namespaced-extension is how the engine adds its needs without forking the spec.

### Extract vs. generate, and the fidelity report

The brand-book workflow above quietly runs two operations that must not be conflated, because the distinction is where the accessibility value is decided. **Extraction is descriptive** — a tool observes real brand assets and emits a snapshot of what is there (these colours, this type, this spacing). **Generation is generative** — an engine takes a few *anchors* from that snapshot and produces a complete, accessible, moded system (the full ramp, the semantic layer, light/dark modes, the `on-*` foreground pairs). The practice's POV, stated plainly: **the accessibility value lives in generation, not extraction — you cannot observe your way to an accessible system.** What a brand ships is almost always incomplete and often inaccessible: a primary and two accents, not a contrast-verified eleven-step ramp; no dark surfaces; a primary that fails contrast on white, so a naïvely *extracted* `on-primary` would fail. The generator is what *produces* the contrast-anchored ramp, the modes, and the luminance-flipped `on-*` pairs (see 31) — it is the Generative Token pattern named as a pipeline stage (see 24). Extraction gives you anchors; generation gives you the system.

This sharpens the "AI as critic, not author" line at the top of this section: the extractor is a *describer*, not an author, and the engine's generation logic — not the AI's guess — is what makes the system accessible. It also sets the boundary in an **owned multi-tool toolchain**: each tool must stand alone *and* compose. The extractor stays a complete descriptive tool (a user who stops there still gets a usable snapshot); the engine simply does not *trust* the extractor's derived ramps as final — it regenerates from the anchors. The principle underneath is **one source of computation**: any given piece of logic is derived in exactly one place. The failure mode is two tools each "helpfully" generating a ramp and drifting apart, so the output depends on which ran last.

Generating from anchors carries one risk worth a named safeguard: the generated value silently diverging from the brand's real one (the engine's step 5 is not quite the blue anyone uses). The **fidelity report** closes it — pin the exact brand anchor, generate the ramp, then emit a report of every place the generated value diverges from the observed one. Nothing changes behind the practitioner's back; every deviation is surfaced for a decision. The right framing is a **regression artefact** — a diff, in the same family as the Cascade Report (see 24) — that lives at build time as a reviewable output and, ideally, a CI gate: a generation run that drifts from the pinned brand beyond a threshold warns or fails, the way a contrast violation does. The fidelity report is what makes "generate the system from anchors" safe to run repeatedly.

**Validated, not just proposed.** The pattern above is no longer hypothesis. A spike ran a real, spec-conformant `design.md` — a full brand extraction, twenty-four colour swatches and twenty-five type roles — through the generation engine end-to-end, and the fidelity report is a working build artefact, not a sketch (Prism3 Wendy's spike, 2026). The measured result is exactly the split the POV claims. The pinned brand anchor reproduced at **ΔE00 0.00** — exact-anchor preservation held — while the *generated* ramp, status, and neutral steps diverged from the observed swatches at an aggregate ΔE00 of about two, the divergence the report exists to surface and put to a decision rather than bury. A small colour-role classifier read the flat `colors:` map into engine anchors by naming convention alone (`primary` / `neutral-<step>` / `success` / `warning` / `error`→`danger`) — the one genuinely new parser the bridge needed. And the namespaced-extension discipline this section prescribes proved out literally: an `x-prism3` block, invisible to the base spec, carried the engine-only levers and round-tripped — the extractor emits it, the engine consumes it, neither forks the format. The accessibility claim landed hardest. The extracted brief asserted its brand red cleared "~4.6:1" on white and prescribed a darker shade for small text; the engine *measured* the actual figure at **5.88:1** — the brief's own contrast prose was stale for its own hex, and only per-contract measurement caught it. The same brief shipped a destructive red identical to a brand shade and an "info" colour identical to its secondary; the engine carved a distinct danger and synthesised the missing accessible info role the brand never supplied. This is "you cannot observe your way to an accessible system" made literal: the describer stated a contrast that was wrong, and *generation*, not extraction, is what held the contract.

*(Provenance: the `design.md` structure, licence, and alpha status are verified against `github.com/google-labs-code/design.md` (fetched 2026-07-01); the interchange-layering, the adopt-internally verdict, the extract-vs-generate split, and the fidelity-report pattern are the practice's POV synthesised from the Prism3 E2E build, single-sourced via the Claude-side run in `_research/_inbound/2026-07-01-generative-brand-pipeline/` — no external-agent pass. The extract-vs-generate split and the fidelity-report + namespaced-extension patterns are now **build-validated** by the Prism3 Wendy's spike (`engine/spike-wendys.ts` + `engine/wendys-fidelity-report.md`), recorded in `_research/_inbound/2026-07-01-generative-brand-pipeline/build-validation.md`. design.md is alpha and moving; re-verify before anything is built or promised on it.)*
...
The docs surface of this layer — the per-component template that makes the structured DS data legible to all four audiences (designers, developers, AI agents, auditors), the seam between generated technical facts and authored intent, the CI freshness check that prevents drift between code and metadata — is covered in 29-per-component-documentation-template. The template's accessibility section maps 1:1 to the 17 / 28 annotation contract; the conformance dossier slot aggregates into the consuming product's ACR. The `.ai.json` schema and the per-component documentation are the same artefact at two altitudes. The commercial-default angle of the pipeline — the engineering investment, the upfront cost split across Phase 1 / 2 / 3 (with custom MCP server as Phase 3), the per-component hours redistribution, the 5-question decision tree, the phased-adoption pitch — is in 30-generated-from-data-documentation, which closes the residual gap between architecture and SOW.
