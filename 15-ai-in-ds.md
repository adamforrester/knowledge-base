...
>
> Scope note: this file is the (a)-axis of the AI-in-DS split — AI features the design system *operates internally* (token critique, codegen, MCP-served metadata, drift detection). The sibling (b)-axis — AI features the design system *ships as patterns to consumers* (chat input, streaming response, citation, refusal, tool-call rendering) — lives in 25-ai-aware-patterns-and-conversational-ui. The two are easy to conflate in scoping conversations and meaningfully different in what they require to ship.
