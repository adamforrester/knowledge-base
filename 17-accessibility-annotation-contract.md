...
**What the engineer implements.** Flutter: `Semantics(label: ...)`. Compose: `contentDescription`. SwiftUI: `.accessibilityLabel(_:)`. UIKit: `accessibilityLabel`. React Native: `accessibilityLabel`.
...
**What happens when missing.** The framework either falls back to the visible text (often correct, sometimes embarrassing — "Add to favourites button button"), reads "image" with no description (failure), or reads the asset filename ("ic_heart_outline_24"). The default is always wrong for production.
...
**What the engineer implements.** Flutter: flags on `Semantics` (`button: true`, `header: true`) plus `SemanticsRole` where supported. Compose: `Modifier.semantics { role = Role.Button }`. SwiftUI: `.accessibilityAddTraits(.isButton)`. UIKit: `accessibilityTraits = .button`. React Native: `accessibilityRole = 'button'`.
...
**What the engineer implements.** Flutter: `Semantics(selected: true, enabled: false, checked: true, toggled: true)`. Compose: `Modifier.semantics { selected = true; stateDescription = "Connecting" }`. SwiftUI: trait combinations (`.isSelected`, `.isToggle`) plus `.accessibilityValue(_:)` for state strings. UIKit: combine `accessibilityTraits` with `accessibilityValue`. React Native: `accessibilityState = { selected: true, disabled: false, expanded: true, busy: false }`.
...
**What the engineer implements.** SwiftUI: `.accessibilityIgnoresInvertColors(true)`. UIKit: `accessibilityIgnoresInvertColors = true`. React Native: `accessibilityIgnoresInvertColors` on a wrapper `<View>` (the prop is unreliable on `<Image>` directly). Flutter: platform channel — no first-class framework API. Compose: not directly supported; for Android forced colors, teams build a `CompositionLocal`.
...
**What the engineer implements.** Flutter: `MediaQuery.of(context).disableAnimations` and `accessibleNavigation`. Compose: gate animations on `Settings.Global.ANIMATOR_DURATION_SCALE`. SwiftUI: `@Environment(\.accessibilityReduceMotion)` plus conditional `withAnimation`. UIKit: `UIAccessibility.isReduceMotionEnabled`. React Native: `AccessibilityInfo.isReduceMotionEnabled()` plus Reanimated's `useReducedMotion` hook.
...
**What the engineer implements.** Flutter: `Semantics(onIncrease: ..., onDecrease: ..., customSemanticsActions: ...)`. Compose: `customActions` in semantics. SwiftUI: `.accessibilityAction(named: handler:)`. UIKit: `accessibilityCustomActions = [...]`. React Native: `accessibilityActions = [...]` with `onAccessibilityAction` handler.
...
The annotation fields above translate to ARIA on web, but the spelling and the available primitives differ enough that web teams should not consume this document as-is. The current state for web teams using this contract:

- Treat the **name, role, state, value** fields as directly applicable; the web implementation is `aria-label`, `role`, `aria-checked` / `aria-selected` / `aria-expanded` / `aria-disabled`, `aria-valuenow` / `aria-valuemin` / `aria-valuemax`.
- Treat the **merging and traversal** fields as informative; the web equivalents are `aria-hidden`, `aria-owns`, `tabindex`, and ARIA APG patterns, with materially different ergonomics.
- Treat the **dynamic-type / system-inversion / motion** fields as conceptually parallel but mechanically different — the web uses CSS media queries (`prefers-reduced-motion`, `prefers-contrast`, `forced-colors`) and CSS logical properties for scaling, none of which have direct mobile-framework equivalents.
- Treat the **gesture alternative** field as largely native to web (most web gestures are already pointer-only) — but the principle holds for any drag, swipe, or motion interaction the design specifies.
- Treat the **focus management around transitions** field as critical and directly applicable; web's `inert` attribute, `focus-visible`, and route-change focus management are the implementation surface.
...
For the framework-level implementation depth underneath each annotation field, see 16-mobile-accessibility-implementation.
...
For the components-as-data approach that underwrites Pattern B (annotation as structured metadata), see 03-component-library and 15-ai-in-ds.

---

## Provenance

Pulled out of the synthesis that produced 16-mobile-accessibility-implementation. The annotation fields are derived from cross-framework consensus across four dual-agent research runs filed 2026-05-13. Source files in `_research/_inbound/2026-05-13-mobile-a11y-*/`. Web extension is a known gap — the contract is mobile-scoped today and explicitly so.
