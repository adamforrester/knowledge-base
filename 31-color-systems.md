...
- *CSS Color 4 default (chroma-reduction)* — keep L and H, reduce C until the color is in gamut. Default for `oklch()` colors out of gamut. CSS Color 4 §14 names this as the conceptual approach (`Closest Color (MINDE)`, `Chroma Reduction`, `Chroma Reduction with Local Clipping`) and §14.2 specifies three named algorithms — `Binary Search Gamut Mapping with Local MINDE`, `EdgeSeeker Gamut Mapping`, `Ray Trace Gamut Mapping`. The spec's §20 defines ΔE_OK as a distance metric used elsewhere in the spec. The *category* (chroma-reduction in OKLCh/Oklab space) is verified; which of the three §14.2 algorithms is normatively designated as the CSS default is a body-text question we have not yet captured verbatim from the spec. (Source: `_source-text/ext-css-color-4-gamut-mapping.txt`; the body text of §14.2 was repeatedly truncated by automated fetch and needs a human read.)
...
*Provenance (June 2026 source-text pass — 09 §1.27 closure): three of the four flags surfaced by the original run are now closed with primary-source captures. (1) **APCA Lc threshold table** — captured at `_source-text/ext-apca-thresholds.txt` from `readtech.org/ARC/tests/visual-readability-contrast/`; the §3 tier table now reflects the canonical font-size × weight × Lc lookup matrix, including the correction that the "14px @ 400 weight" figure is Lc 80–85, not Lc 90+. The Lc 30 floor for non-text UI is flagged as practitioner shorthand pending Myndex's published Non-Text Contrast guideline. (2) **DTCG `colorSpace: "display-p3"`** — captured at `_source-text/ext-dtcg-format-module.txt` from the Color Module 2025.10 (13 June 2026 preview draft); fourteen colorSpace values enumerated, `display-p3` confirmed; §1 line de-hedged. (3) **Style Dictionary `color-mix()` watch item** — captured at `_source-text/ext-style-dictionary-changelog.txt` (current version 5.4.4); no native preservation; recommendation in §1 holds; next quarterly check 2026-09-15. The remaining flag (4) **MINDE Chroma Reduction algorithm name** — captured partial at `_source-text/ext-css-color-4-gamut-mapping.txt`; the spec's TOC structure verifies that `Closest Color (MINDE)`, `Binary Search Gamut Mapping with Local MINDE`, `EdgeSeeker`, and `Ray Trace` are named §14 subsections, and that ΔE_OK is a defined distance metric in §20, but the §14.2 body-text designation of which algorithm is the CSS default exceeds automated-fetch windows. The §1 line is now precise about what's verified (TOC structure, algorithm names, ΔE_OK metric existence) and what isn't (which §14.2 algorithm is normatively the CSS default). Promote to a 5-minute human-fetch task at the next vault session — see 09 §1.27a residual.*
