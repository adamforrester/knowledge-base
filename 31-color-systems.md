...
### Gradients — the category most systems deliberately skip

Gradients are the most-skipped token category in the field, and the first useful thing to say about them is that **abstaining is a defensible position, not an oversight.** Of ten systems surveyed, the majority ship no gradient tokens at all: Material 3, USWDS, Primer, and Salesforce SLDS 2 abstain outright; IBM Carbon ships SCSS mixins (`ai-gradient()`) rather than tokens; Atlassian composes gradients in components with no dedicated token category. Shopify Polaris *had* a `bg-gradient` token and **removed** it. The judgement across the field is that gradients are too contextual to standardise as primitives — they belong to a hero, a chart, an AI affordance, not to the palette. **So the practice's default is thin and opt-in: ship gradient tokens only when a brand actually declares gradients as part of its identity, and don't make them a derived-for-everyone axis.** Where a brand does, three constraints shape the encoding.

**The DTCG `gradient` type is real but stops-only, and the gap is the interesting part.** In the Format Module 2025.10, `$value` is an array of `{ color, position }` stops where a stop's `color` may be a token reference and `position` is a number in [0,1]. What it does *not* encode: **kind (linear/radial/conic), angle or direction, radial centre and size, or interpolation colour space.** That omission is known and still open — issue #101 has collected exactly these gaps since 2022 and is labelled a next-draft priority; the 2025.10 release added OKLCH and Display-P3 to the *colour* type but left gradient untouched. The practical consequence: a DTCG gradient token is half a gradient, and the missing half has to live in `$extensions` — which is the materialization directive from 22 applied to a spec hole rather than a tool quirk.

**Stops must alias the colour ramp — this is the whole theming mechanism.** Microsoft Fluent is the mature model (a decomposed composite: `start`/`end` points plus `stops[]` where each stop carries `aliasOf`), and Carbon's mixins do the same thing in SCSS. The cautionary case is the opposite move: Polaris and SLDS 1 shipped gradients as **opaque CSS strings with raw rgba inside**, which is precisely why both were deprecated — a raw-string gradient cannot theme, cannot flip for dark mode, and cannot participate in a brand swap. **Alias each stop to a theme-aware semantic token and the gradient inverts for free**; author separate light and dark gradients and you have doubled a maintenance surface for no benefit.

**Interpolation is where design and code diverge, and the fix is a pre-sample.** CSS supports `linear-gradient(in oklch, …)` (Baseline since mid-2023; CSS now defaults to Oklab), which avoids the grey dead zone a two-stop sRGB interpolation produces through the midpoint — the same perceptual argument that made OKLCH the authoring space in §2. **Figma interpolates in sRGB only**, with no perceptual option in the UI or API. So an OKLCH gradient will not reproduce faithfully in Figma; its midpoint reads greyer. The reconciliation is to hold OKLCH as the canonical definition and **pre-sample the curve into N baked sRGB stops** for the Figma side — an instance of the same author-canonical / materialise-per-target discipline the line-height and shadow work already follow (see 12-figma-practice for the Paint Style mechanics: a gradient is a `Paint`, never a variable; only stop *colours* bind, while kind, angle and positions are baked; and Paint Styles are plugin-only to create).

**One accessibility rule, which no surveyed system performs.** Text over a gradient must meet its contrast ratio at the **worst-case point under the text**, not on average — the standard falls under WCAG 1.4.3/1.4.11 with no gradient-specific guidance. The practical encodings are to constrain the gradient's lightness range, add a scrim, or put the text in a solid container; the systemic one is to **compute contrast at the worst stop as a build-time gate**, which is a genuinely differentiating check given nobody in the survey ships it. Watch for silent gamut clipping when interpolating high-chroma OKLCH (see §2 on P3 and clamping).

*(Sourced from the gradient-tokens research run, `_research/_inbound/2026-06-29-gradient-tokens`, single-sourced — Claude-side only, no external-agent pass — and accepted on the strength of its primary-source survey: DTCG 2025.10 and issue #101, ten systems' published tokens, and the Figma Plugin/REST references. The abstention finding, the Fluent-vs-Polaris split, the stops-only gap, and the sRGB/OKLCH mismatch are sourced; the thin-and-opt-in default and the worst-stop contrast gate are the practice's judgement.)*
...
**The shadow-fade-becomes-surface-lighten swap.** In light mode `shadow/large` is a soft drop shadow. In dark mode the equivalent token is *the same shadow plus a surface lift*, or — if the system commits — the shadow disappears entirely and the surface lift carries the elevation. Both patterns are in the field; M3 keeps a faint shadow plus surface lift; some systems (Vercel's, Stripe's) use surface-only. **The practice's stance: lift-primary, with the shadow reduced but not nulled.** Dark elevation is carried mainly by the lighter surface (shadows are hard to see on a dark ground), but M3 and Atlassian both retain a faint shadow at the top steps and we follow them — keeping a reduced shadow preserves the edge that pure surface-lift loses on a busy background. We explicitly reject the *heavier*-shadow-in-dark branch (Fluent and some others push opacity up — e.g. 14%→28%); it is the minority pattern and it muddies dark UIs rather than clarifying them.
...
**Nice-to-have fields.** `tonal_palette_siblings` (linking each color in a ramp to its neighbours in the same hue, so a model picking step 9 can reach for step 8 hover and step 11 text without re-querying); `forced_colors_mapping` (the system keyword the token resolves to); `gamut_clamps` (the sRGB and P3 versions when the source is OKLCH); `aliased_by` (the *reverse* index — every semantic and component token that resolves to this primitive, computed **transitively** through multi-hop alias chains, so a primitive shows its full blast radius before anyone changes it). The last is generated, not authored — the agent-readable counterpart to the Cascade Report in 24-tokens-at-scale, and useful to a human maintainer for exactly the same impact-analysis reason.
