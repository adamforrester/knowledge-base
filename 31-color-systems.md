...
- *CSS Color 4 default (chroma-reduction)* — keep L and H, reduce C until the color is in gamut. Default for `oklch()` colors out of gamut. The external pass names a specific algorithm — *MINDE Chroma Reduction with ΔE_ok distancing* — flag this as worth verifying against the CSS Color 4 spec or implementation notes before quoting; the *category* (chroma-reduction in OKLCH/Oklab space) is correct, the algorithm name needs `_source-text/` backing.
...
APCA (Accessible Perceptual Contrast Algorithm) returns an Lc value, not a ratio. It's *asymmetric* — light text on dark and dark text on light yield different Lc because perceptual contrast is direction-dependent. Lc tiers (mid-2026, the external pass's table is the more specific source — flag for `_source-text/` verification before quoting verbatim):

| Lc tier | Intended UI application | Approximate typography minimum |
|---|---|---|
| **Lc 90+** | Critical UI, super-bold text — the highest tier. | 14px @ 400 weight (the external pass's specifics). |
| **Lc 75–90** | Body-text minimum (analogous to WCAG AA but perceptually grounded). | 18px @ 400 or 14px @ 700. |
| **Lc 60–75** | Large or non-essential text, UI labels. | 16px+. |
| **Lc 45–60** | Large headings, decorative. | 36px @ normal or 24px @ bold. |
| **Lc <45 / Lc 30 floor** | Non-text interactive UI components, borders, icons, focus rings. | Below 30 fails for any text. |
