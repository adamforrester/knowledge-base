...
- **No-results / filtered-out.** A query was made and returned nothing, or filters reduced everything away. **Empty State, no illustration, secondary "Clear filters" CTA.** Cloudscape (AWS) explicitly *mandates* this shape — a secondary-styled "Clear filter" button is the prescribed action; an illustration would be jarring. Voice: clarifying ("0 of 247 results match. Clear filters?").
...
relations:
  ...
  relates-to: [notification-orchestration, error-pages, onboarding, forms-as-a-system, bulk-actions-and-selection]
  boundary: >
