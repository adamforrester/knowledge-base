...
- **Forms-as-a-system vs the data-collection "Ask users for X" recipes** (see patterns/data-collection-recipes) — this pattern is the orchestration *shell*; the per-datum recipes (address, name, date-of-birth, payment card — the "3 inputs vs 1" decisions, the autocomplete-token contract, the i18n shape) are a sibling `composition` pattern composed *inside* the shell, not absorbed.
