...
- **[Forms-as-a-system](forms-as-a-system.md)** — `goal: ask-for-data.` The multi-field form as a system: reward-early/punish-late validation timing, the dual inline + error-summary model and its focus contract, mark-the-minority required/optional, the sectioned/one-per-page/wizard threshold, conditional reveal, cross-field validation, the save model, single-column layout, and the never-disable-submit contract. Composes Form + every control + Button + Inline Message; borders the multi-step wizard (flow), the "Ask users for X" recipes it composes, and the state/token rules in Foundations. **The patterns layer's first brief** — the trigger; exercises the most of the spine.
- **[Data-collection recipes](data-collection-recipes.md)** — `goal: ask-for-data.` The "Ask users for X" family — the per-datum field recipes (name, address, date of birth, phone, email, payment card) composed inside the Forms-as-a-system shell. The cross-cutting spine — one-input-vs-many, the non-negotiable autocomplete-token contract (with the third-party-data caveat), tolerate-and-normalise, the i18n-first posture, proportionality — plus a per-datum recipe table with each datum's signature anti-pattern (the date-picker-for-DOB being the sharpest, doubling as the Date Picker boundary). Composes Text Field / Select / Fieldset / Date Picker; borders Forms-as-a-system (the shell composes it), the Date Picker component, the Login pattern, and 13-internationalization. **Validates the `goal` facet** — the highest-field-attestation pattern, anchored on the GOV.UK corpus.
...
- **[Search orchestration](search-orchestration.md)** — `goal: navigate.` The search experience around the Search field component — the zero-query state, the suggestion model (query-suggestions vs result-previews vs federated), scoped/autodirect search, the results surface, and (above all) the no-results state, plus the instant-vs-submit decision with its debounce + AbortController out-of-order-response guard and the AT announcement budget (the separate live region, never aria-live on the listbox). Composes Combobox / Popover / Empty State / List / Spinner; borders the Search field component (the input), Filtering (search produces the set, filters refine — faceted search), View-state orchestration (the no-results empty instance), the Command palette (search-as-action-launcher), and results pagination. The field is the component; this is the orchestration.
- **[View-state orchestration](view-state-orchestration.md)** — `goal: recover-from-error.` Coordinating a region's data states — the duration-threshold loading taxonomy (skeleton vs spinner vs optimistic vs nothing at 300ms / 1s / 10s), the five-category empty-state taxonomy, the four-axis error-state taxonomy at region scale, layout-preserve-vs-replace, the live-region budget with politeness creep as the named failure mode, the optimistic + rollback contract with destructive/financial/async-by-design carve-outs, and the skeleton-faithfulness governance rule. Composes Empty State / Spinner / Skeleton / Banner / Toast / Inline Message; reads from Table / List / Card / Grid. **The worked example for the schema's foundations-exile distinction** — the brief that validated the orchestration-as-`composition` call against the bare-state-rule-is-Foundations alternative.
...
- **[Destructive confirmation](destructive-confirmation.md)** — `goal: help-user-act.` Guarding an irreversible/high-cost action — and the question at its heart, confirm vs undo (the undo-first default; over-confirmation is itself the defect). The five-rung friction ladder (no friction → undo toast → confirm dialog → type-to-confirm → MFA/approver), the alertdialog focus contract (never autofocus the destructive button), action-verb labels, the soft-delete grace-period model and its timeline, and the copy that carries the safety. Composes Dialog/alertdialog / Button / Toast / Banner / Inline Message; borders the Dialog component, the Toast component (the undo surface vs the undo model), Save & unsaved-changes (the sibling discard-guard flow), and Forms-as-a-system. **The first `flow`-tier brief** — flow & states carries the weight, layout is light.
...
- **[App shell / global header](app-shell.md)** — `goal: navigate.` The persistent application frame every screen sits inside — the global header's three zones, the top-vs-side-vs-hybrid nav fork, the desktop→tablet→mobile responsive collapse (drawer vs bottom-nav, Priority+ overflow), the landmark/skip-link skeleton (the skip target needs `tabindex="-1"`), and the SPA route-change focus/scroll/announce contract (the most-missed SPA a11y failure). Composes Side Navigation / Menu / Link / Avatar + the search slot; borders the Side Navigation component (the control vs the frame), the page templates that sit inside it, Search orchestration (the header slot), and the IA strategy. **The first `layout-and-shell` brief** — layout carries the weight, flow is light; the substrate every other archetype sits in.
...
## By goal — the task facet

The structural tiers above answer "what shape is the solution"; this cross-index answers "**what is the user trying to do**" — the GOV.UK-style `goal` facet carried by every pattern, the axis a reader (or the planned MCP `list_patterns()`) reaches for when the question is intent, not structure. Briefed patterns link; `*` marks not-yet-briefed stubs (see the tiered catalogue above for scope).

- **`ask-for-data`** — collect structured input. [Forms-as-a-system](forms-as-a-system.md) · [Data-collection recipes](data-collection-recipes.md) · Inline edit* · Multi-step wizard*
- **`navigate`** — get around, find a destination. [Search orchestration](search-orchestration.md) · [App shell / global header](app-shell.md) · Onboarding*
- **`show-data`** — present and explore data the user reads. Filtering* · Data visualization* · Master–detail* · Dashboard* · Feed & supporting-pane*
- **`recover-from-error`** — states, failures, and getting unstuck. [View-state orchestration](view-state-orchestration.md) · Notification orchestration* · Error pages*
- **`help-user-act`** — execute and guard an action. [Destructive confirmation](destructive-confirmation.md) · Action priority & overflow* · Bulk actions & selection* · Login & authentication* · Save & unsaved-changes* · Settings* · AI conversational flow*
- **`help-user-decide`** — support a choice. Comparison & pricing tables* · Check answers* · Eligibility / triage*

A pattern carries exactly one primary `goal` (its dominant intent); the structural tier and this facet are orthogonal axes over the same set, not a hierarchy.

---

## Provenance

The patterns layer and this index were opened after the component catalogue closed its seventh category (Form). The flexible spine is defined in `_schema.md`. **Forms-as-a-system** is the first brief — the trigger; the Form component forward-referenced it as the pattern boundary; it exercises the most of the spine. **View-state orchestration** is the second brief — the worked example for the schema's foundations-exile distinction; ran in parallel via the practice's standard dual-agent flow at ~94% convergence; validates the orchestration-as-`composition` call against the bare-state-rule-is-Foundations alternative.

As of this consolidation pass, **seven briefs are shipped** and the layer has exercised all three tiers — `composition` (Forms-as-a-system, Data-collection recipes, Search orchestration, View-state orchestration), `layout-and-shell` (App shell), and `flow` (Destructive confirmation) — against **one unchanged schema**: the tier-conditional spine bent decision-heavy, layout-heavy, and flow-heavy in turn without needing a new key, which is the signal the scaffolding was right to freeze before parallel authoring. The cross-reference graph among the briefed set is reciprocal (the no-results seam runs both ways between Search and View-state; the undo/optimistic seam between Destructive confirmation and View-state). See `03-component-library` for the practice's POV on the component model the patterns compose, and `CLAUDE.md` for the vault conventions.
