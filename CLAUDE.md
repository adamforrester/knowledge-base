...
- `25-ai-aware-patterns-and-conversational-ui.md` — the (b)-axis sibling to 15: AI patterns the DS *ships to consumers* (chat, streaming, citation, refusal, tool-call). Field survey of the eighteen named DSs plus extensions, production-grade triad (Cloudscape / Atlassian Rovo / Twilio Paste), eBay MIND as accessibility-not-AI, Vercel AI Elements as the leading open primitive layer, seven-cluster taxonomy with tool-call broken out, EU AI Act Article 50 and US state disclosure regimes, the four-prerequisite minimum applied to the consumer surface.
- `26-adaptive-interfaces-foundations.md` — the third axis after 15 (AI operating the DS) and 25 (AI patterns the DS ships): interfaces that *reshape themselves* in real time. Six lineages and why naming them is the first move, Sentient Design (Clark & Kindred 2026) as the practice's adopted vocabulary, the Sentient Triangle scaffold and four postures, the four-tier pattern catalogue (14 experience / 8 functional / 7 interaction / 17 UI patterns; 46 named patterns total), the agent typology (Taskers as baseline; Alchemists, Stewards, Conductors as specialisations) with the workflow-vs-notebook control axis, personalization vs. adaptation as the load-bearing diagnostic, ability-based design as the strongest empirical case, the e-commerce-breaks-the-binary reframe, the agentic-commerce traffic shift (Adobe ~693% YoY), EU AI Act Article 5, the second manipulation vector (LLMs hypersensitive to nudges), the Action Guard pattern and dialog-fatigue rule, eight practice positions to commit to.
- `27-adaptive-interfaces-implementation.md` — the engineering companion to 26: five-rung spectrum (rule-based / catalog / template / structured-output / code-emit), tool-calls as UI primitive, structured outputs and constrained decoding, RAG as upstream context, multi-step agentic flows with workflow-vs-notebook architecture and the 3–6 tool-count rule, the interface-agent caveat (path of most resistance, not the future), the multimodal map with cross-device handoff as the under-solved one, the four-tier memory architecture (session / profile / vector / context window) and why architecture is governance, streaming UI as a primitive, eval-driven QA with accessibility as the weakest link (25–40% automation ceiling), cost modelling (12× swing across model tiers), the authoring-tool gap and "variability by design," design-system implications and the case for variant-logging-as-infrastructure as a Prism2 extension.
