...
- `26-adaptive-interfaces-foundations.md` — the third axis after 15 (AI operating the DS) and 25 (AI patterns the DS ships): interfaces that *reshape themselves* in real time. Six lineages and why naming them is the first move, Sentient Design (Clark & Kindred 2026) as the practice's adopted vocabulary, the Sentient Triangle scaffold and four postures, fourteen experience patterns, personalization vs. adaptation as the load-bearing diagnostic, ability-based design as the strongest empirical case, the e-commerce-breaks-the-binary reframe, the agentic-commerce traffic shift (Adobe ~693% YoY), EU AI Act Article 5 and the second manipulation vector (LLMs hypersensitive to nudges), eight practice positions to commit to.
- `27-adaptive-interfaces-implementation.md` — the engineering companion to 26: five-rung spectrum (rule-based / catalog / template / structured-output / code-emit), tool-calls as UI primitive, structured outputs and constrained decoding, RAG as upstream context, multi-step agentic flows, the multimodal map with cross-device handoff as the under-solved one, the four-tier memory architecture (session / profile / vector / context window) and why architecture is governance, streaming UI as a primitive, eval-driven QA with accessibility as the weakest link (25–40% automation ceiling), cost modelling (12× swing across model tiers), the authoring-tool gap and "variability by design," design-system implications and the case for variant-logging-as-infrastructure as a Prism2 extension.
