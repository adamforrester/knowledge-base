...
Automated scanning catches roughly **30–40% of WCAG conformance issues** (per Deque's published self-assessment of axe-core's coverage; the figure is widely cited but not trivially verified — flag for source-text backing per §1.29a below). The remaining **60–70% require manual AT testing** — the four-engine × four-AT matrix per 32 §2.3 plus 28's web-accessibility-implementation framing.
...
- **Specify MCP**, **Knapsack MCP**, **Supernova MCP**. Each vendor exposes a programmatic API; MCP wrappers exist (custom or community) at varying maturity. None ships native MCP as of mid-2026; verify against current vendor docs.
...
- **No major design-token vendor ships native MCP as of mid-2026.** Specify, Knapsack, Supernova, Backlight all have APIs that an MCP wrapper can consume; none ships MCP natively. The practice's bet: native MCP lands in 2026–2027 — AI-readiness pressure is real, engineering work is bounded, commercial signal is clear.
