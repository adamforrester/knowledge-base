...
The external agent flags **expression tokens** as a related capability — Figma variables that allow conditional logic and math-driven computation, "rolling out in late 2025." *(External only — claim should be verified against current Figma docs; if real, materially changes how dynamic theming is encoded in tokens.)* If they're stable, AI agents can simulate token permutations across modes and surface accessibility regressions automatically. **Treat as "verify before recommending"** until our own pilot confirms the feature surface and stability.
...
- **Hard-coded values in consuming code.** Hex codes, pixel values, magic numbers that should be aliased through tokens. Tools like **Design Lint** *(external only as a specific named tool)* have evolved to include semantic understanding for this.
...
*(External agent commits to specific maturity labels for these; Claude's pass hedged. Synthesis preserves the labels as "reported as" — useful directional signal, but flag for verification.)*
...
The official server is **primarily optimized for read-only operations** when used with personal access tokens *(both agents converge on this caveat)*. Advanced write capabilities require additional plugins or alternative servers.

### Figma Console MCP and write access

A separate MCP implementation, **Figma Console MCP** (Southleft) *(external only on attribution — verifiable; the figma-console MCP server appears in our own tooling environment)*, extends the official capabilities by providing project-wide context and full **write access** through a "Desktop Bridge" plugin and a "bootloader" architecture. This allows AI agents to write directly to the Figma canvas — creating frames, components, and variables through natural language commands, applying token migrations across thousands of nodes, generating exemplar pages from component metadata.
...
The practical workflow with MCP, in increasing order of context maturity *(Claude only — useful framing for client conversations; preserves the architectural argument)*:
...
Where human judgment is still required *(both agents)*:
...
The thesis, sharpened: **AI-in-DS workflows fail or succeed on whether the component is defined as structured data.** *(Both agents converge on this as the central architectural claim; Curtis's *Components as Data* essay is the canonical reference. The vault's 03-component-library already names this as the post-tokens architectural move.)*
...
Nathan Curtis's **Specs CLI** (DirectedEdges/specs on GitHub) operationalizes the components-as-data POV. The architecture *(both agents)*:
...
The workflow has several stages *(external only on the specific subcommand names; Claude described generally; external's specifics adopted as useful detail)*:
...
The dominant AI coding assistants in 2026 — Claude Code, Cursor, Copilot, Windsurf — all support MCP and all are usable for DS component work. *(both agents)*
...
The shift in prompting style: rather than "implement a blue button," practitioners now prompt with system references — "Using the `Button` anatomy and the `color/action/primary` collection from our MCP context, implement a React component supporting a disabled state with `aria-label` for icons." *(both agents.)* The result is production-ready code rather than what the external agent calls "AI slop" — code that looks correct but uses invented component names and hardcoded values.
...
The external agent flags **"Smart Variant Adaptation"** *(external only — needs verification as a current named feature)* — the pattern of generating responsive variants (mobile, tablet) automatically from a desktop base implementation. The underlying capability is plausible and useful if available; verify the specific feature naming before committing to it in client materials.
...
Storybook has become a critical node in the AI-ready DS. The external agent flags **`experimentalComponentsManifest`** *(external only — verify the specific feature flag name against current Storybook docs)* — a Storybook feature that generates a machine-readable package of component metadata: APIs with validated prop types, usage examples from stories, JSDoc descriptions, test suites that encode requirements.
...
The external agent cites a **30% time recovery** for developers using Code Connect *(external only — figure may be from Figma marketing; cite the source rather than asserting if used in client materials)*. The directional claim is consistent with practitioner reports; the specific number warrants attribution.
...
This works in proof-of-concept and is appearing in some commercial tools. The external agent names **"Instance Finder" and "Token Master"** as specific tools *(external only — needs verification as current named tooling)*. The accuracy is uneven — false positives common — and the report typically needs human triage before becoming an actionable backlog. **Worth piloting; not yet worth selling as a polished offering.**
...
AI-generated changelogs from git diffs are reliably useful — produces a draft covering the changes, typically requires light editing for tone and to elevate user-facing impact over technical change. *(both agents.)*
...
The token side has stabilized around the **W3C DTCG format** — a JSON schema for tokens that's machine-parseable, type-aware, and supports the full range of token types (color, dimension, duration, shadow, gradient, typography composite, etc.). *(both agents)*
...
A reinforcing detail: token names like `surface/action/primary/rest` are AI-readable specifically because they read as compressed sentences. The semantic structure carries intent. **Visual names** (`blue-500`) **strip intent and force AI to extrapolate.** *(both agents converge.)* This is the same point made in 02-design-foundations on naming as compressed sentences — restated here because the AI-readability of the system depends on it.
...
- **AI slop.** UI that looks correct but is built with poor technical foundations. Without components-as-data, AI hallucinates props or relies on generic patterns that don't match the codebase. *(Both agents flag this; "AI slop" is the external's phrase, useful enough to keep.)*
...
- **Token-cost economics.** *(External only — useful operational note.)* If an agent is forced to load an entire massive design system into context for every task, costs and latency compound. The MCP-server architecture is partly a response to this — give the agent queryable access rather than dumping everything in context.
...
- **Skill atrophy.** *(Claude only — keep.)* Engineers and designers who delegate routine work to AI may not develop the underlying skills. Real concern for junior practitioners; less so for senior ones with the skills already.
- **Vendor lock-in.** *(Claude only — keep.)* Commitments to specific AI tools, MCP servers, or integrations create dependencies that may not age well as tooling churns.
- **Cost.** *(Claude only — keep.)* Frontier AI tools used at scale across a team cost real money. Budgets need to anticipate this.
- **The flood-of-low-quality-contributions risk.** *(External only — useful framing.)* AI-assisted contribution lowers the friction of submitting components, which can flood the system with low-quality or redundant proposals if the gate isn't strong. The role of the human "system architect" — setting standards, reviewing logic, ensuring coherence — becomes more critical, not less.
...
- **The senior practitioner's value shift.** From "how to build" to "what to build and why." *(External framing; we adopt.)* AI handles increasingly complex technical execution; the system architect sets the spirit and the standards.
...
*Provenance: The components-as-data POV is from Nathan Curtis's "Components as Data" essay (Sep 2025) and his Specs CLI repo (github.com/DirectedEdges/specs); both are central references for this file. The MCP standard is from Anthropic (modelcontextprotocol.io); the Figma Dev Mode MCP server is a Figma product (help.figma.com/hc/en-us/articles/32132100833559); the figma-console MCP (Southleft) is a community implementation extending it with write capabilities and a Desktop Bridge plugin. The W3C DTCG token format (design-tokens.github.io/community-group/format/) is the parseable structure underlying machine-readable tokens. Code Connect (figma.com/code-connect-docs/) is the Figma-side mapping between design components and production code components. The "stochastic, incomplete, and imprecise" caveat on Dev Mode MCP without structured component data is Curtis (2024) and is the practical core of the architectural-prerequisite argument. The `.ai.json` schema and four-layer AI stack are internal practice IP from AI-Compatible Design Systems (Feb 2025). Wolosin's 12-field component metadata schema (Indeed, Jun 2025) is a parallel structured-data implementation worth referencing alongside Specs CLI. Specific Figma AI feature claims (First Draft / Visual Search / Smart Page Summaries / Figma Make / Rename with AI as GA or Beta) and MCP-related capability claims (expression tokens, Smart Variant Adaptation, Instance Finder, Token Master, experimentalComponentsManifest) are dated to 2025–2026 in current research and should be verified against current Figma and Storybook documentation before client-facing recommendation. Reported figures (30% time recovery from Code Connect, 60–80% documentation-time savings, share of WCAG issues machine-detectable) should be attributed to source rather than asserted. The three-level context-maturity framing (bare Figma MCP / + Code Connect / + custom DS MCP) and the four-prerequisite minimum architecture (components-as-data, DTCG, Code Connect, component metadata) are positions sharpened from internal practice work and are the central architectural recommendations of this file. The "AI as critic, not author" pattern, the screenshot-vs-context-vs-Code-Connect three-paradigm framing, and the gate-vs-advisory framing for AI in contribution review are practice POVs. AI coding assistants (Claude Code, Cursor, GitHub Copilot, Windsurf) are dominant tools in 2026 but vendor landscape rotates fast. This file was synthesized from a dual-agent research run in May 2026 (see `_research/_archive/2026-05-05-ai-in-ds.md` for the editorial trail).*
