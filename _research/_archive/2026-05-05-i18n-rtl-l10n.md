...
Retrofitting i18n into a system that wasn't built for it is asymmetrically expensive. The work falls in four buckets *(both agents converge on this framing, with the external agent itemizing it as six engineering stages — discovery, documentation, triage, fixing, review, regression)*:
...
*(External agent presents an alternative four-level maturity ladder — Locally Targeted, Globally Aware, Fully Localized, Globally Optimized — that maps roughly onto the same stages with more business-flavored vocabulary. The above stage names are more useful for engineering scoping; the external maturity-ladder vocabulary may be more useful in pitch decks or stakeholder briefs.)*
...
The 2026 best practice for bidirectional layout is to write CSS in **logical properties** — properties that resolve to physical sides based on the document's writing mode and direction. *(both agents converge on this as the established standard.)*
...
**For inline content that bucks the document direction** — a Latin URL inside an Arabic paragraph, a code snippet inside an RTL UI — the right tool is the `<bdi>` element or `dir="ltr"` attribute on a span, plus `unicode-bidi: isolate` in CSS. *(Claude only — external agent didn't cover bidi text controls; this is a real practical tool worth knowing.)* These prevent the bidi algorithm from re-ordering content that has its own internal direction.

### Component mirroring rules

Mirroring is not "flip every pixel." It's "flip what reflects directionality of reading or time; preserve what reflects physical objects, brand, or universal conventions." *(both agents converge on the principle and substantially agree on the catalogue.)*
...
Icons should carry a metadata flag — `directional: true` or `mirror: true` — that the build pipeline reads. Directional icons either ship as two assets (LTR and RTL versions) or render with `transform: scaleX(-1)` applied via the `[dir="rtl"]` selector. The latter is cheaper but breaks if the icon includes embedded text or a brand mark; in those cases ship two assets. *(both agents.)*
...
Figma's native RTL support has improved through 2024–2025 but remains incomplete. *(both agents agree there are gaps, with the external agent more optimistic about mode-based workarounds; Claude more cautious about residual issues. Synthesis lands between, leaning toward Claude's caution because the residual issues bite in production.)*
...
2. **Pseudo-localization** with a bidi pseudo-locale that sets direction *and* expands strings. Catches both directional and expansion issues simultaneously. Firefox has a built-in pseudo-locale flag (the external agent cites `intl.l10n.pseudo` set to `bidi` for this; verify the exact flag name against current Firefox docs as flag names change). Chromium has equivalent capabilities via DevTools.
...
*(External agent's table; broadly aligned with IBM/W3C published guidance. Verify the specific numbers against current published sources before using them in client materials.)*
...
Different scripts have different vertical metrics, descender depths, and density. A type scale tuned for Latin will fail other scripts in predictable ways. *(both agents — Material Design's three-category framing is the established convention.)*
...
A common misunderstanding worth correcting directly: **variable fonts do not solve multi-script support.** *(Claude only — flagged because the external agent's framing implies they do. This is a misunderstanding our practice should call out explicitly.)*
...
The DS should also document **complementary signals** for status that don't rely solely on color: an error icon, a warning icon, a checkmark for success. This is good practice for two reasons at once — color alone failing is also an accessibility issue (color blindness), and it makes status communication robust to cultural color variation. *(Claude's tie-in between cultural color and accessibility — keep, it's a defensible POV.)* (See 02-design-foundations on the accessibility-as-foundational stance.)

### Where formatting lives

Formatting is application concern, full stop. The DS's job is to define the **component slot** that holds formatted output, not the formatting logic. *(both agents converge on this boundary, though Claude draws it more sharply.)*
...
- **Date pickers.** Different locales have different week-start conventions (Sunday in US, Monday in most of Europe, Saturday in some Middle Eastern countries). Different month/year orderings (DD/MM/YYYY, MM/DD/YYYY, YYYY-MM-DD). Different calendar systems (Gregorian, Hijri/Islamic, Buddhist, Japanese imperial era). The DS should provide a date picker whose week-start, format, and calendar are configurable, not baked in. *(Claude went deeper on calendar systems; worth keeping. Most agencies don't ship a fully calendar-system-aware date picker and lean on third-party — Adobe Spectrum's, MUI X's, react-aria's.)*
...
Without intervention, multi-locale Figma libraries double in maintenance burden — every component has an LTR copy and an RTL copy, every typography update happens twice, every spacing change happens twice. **This is the wrong default.** A DS at any scale cannot sustain it. *(both agents agree.)*
...
*(The external agent presents the variable-mode workaround more optimistically; we land more cautious. Modes are a real partial solution and worth implementing, but they don't make Figma the authoritative RTL preview.)*
...
The clearest framing: **DS provides the shell; application provides the content.** *(External agent's framing; Claude's explicit list operationalizes it.)*
...
*Provenance: This file synthesizes Claude's research pass and an external research-agent pass on i18n, l10n, and RTL for design systems (May 2026, see `_research/_inbound/2026-05-05-i18n-rtl-l10n/`). Both agents converged on the major architectural patterns — i18n as an architecture concern, logical properties as the 2026 default, externalized strings + ICU MessageFormat + pseudo-localization as the foundation, mirroring rules tied to reading/temporal direction, the "DS provides the shell, application provides the content" boundary, and the Figma maintenance trap with mode-based partial solves. Specific items unique to the external agent (the four-level "Locally Targeted → Globally Optimized" maturity ladder, the specific text-expansion percentage table, specific Firefox pseudo-locale flag names, the "modes as locales" optimism for Figma) are flagged inline where they affect operational guidance and should be verified against current sources before client-facing recommendation. Items unique to Claude's pass that the synthesis preserves: the variable-fonts-don't-solve-multi-script correction, the bidi text controls (`<bdi>`, `:lang()`, `unicode-bidi: isolate`), the calendar systems concern in date pickers, the address-form structural variation depth, the cultural-color / accessibility tie-in. The "tokens hold design decisions, locale data is application concern" boundary is Claude's framing, sharpened in synthesis as a defensible practice POV. The CSS logical properties table, mirroring rules table, and system font stack table are presented from the convergent material in both passes.*
