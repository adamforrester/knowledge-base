...
Accessibility is not one layer; it's distributed across four. The external agent presents this as a structural map *(external only as an explicit table; Claude's pass had the same decomposition in prose)*; we adopt the table form because it makes the architecture legible:
...
The cost asymmetry is well-known. **A DS that surfaces issues at the constraint layer pays the 10-minute cost; a DS that audits later pays the coordination cost on every issue.** Industry figures vary by source — the external agent cites "5% added design time when built in vs. 15–30% more expensive to fix later" *(external only — broadly aligned with industry benchmarks; cite the source rather than asserting)*; the broader point is that the gap is large and weighted against retrofit.

There is also the **overlay-industry caution** worth surfacing in client conversations *(external only)*. The "automated overlay" market — third-party scripts that promise compliance via JavaScript injection — is increasingly the target of accessibility lawsuits because the scripts conflict with assistive technologies and don't address underlying structural issues. Clients sometimes ask about overlays; the practice should have a clear position that they're not a substitute for built-in accessibility, and that recommending them creates legal exposure rather than reducing it.
...
- **EAA (European Accessibility Act)** became enforceable on **June 28, 2025** *(external only — verifiable; worth keeping with the date)*, applying to products and services sold in the EU including most digital consumer-facing products.
...
The most consequential token-layer decision is whether color tokens encode contrast as a property of the **pair** or leave it as a property of the consumer's combination. **Encoding it as a pair is the only architecture that prevents drift.** *(both agents converge.)*
...
**Status as of early 2026:** APCA is a candidate for WCAG 3 but WCAG 3 itself is not yet stable. APCA can be used as an internal target and surfaces in tooling (Adobe Leonardo, Stark) but is not yet an accepted compliance standard. *(External agent leans more bullish — "design systems in 2026 should incorporate APCA-based testing"; Claude's pass is more cautious. Synthesis lands on the cautious side because APCA is not yet a procurement-grade standard.)*
...
Animations that can't simply be zeroed (a page transition that conveys spatial relationship) should fall back to an instant or near-instant version, or to a different transition style (cross-fade instead of slide). The external agent also names **calm easing tokens** as a parallel concern *(external only — useful refinement)*: providing easing curves that avoid abrupt starts or stops, less likely to cause discomfort even for users without reduced-motion settings enabled.
...
In complex layouts, **intelligent focus rings that change color based on the surface they inhabit** *(external only — useful pattern)* are worth shipping when the design system uses many surface colors. The ring color can be a function of the local surface mode rather than a single global value.
...
The system absorbs ARIA complexity and exposes only high-level props to the consumer. A `Tabs` component generates `role="tablist"`, `role="tab"`, and `role="tabpanel"` internally; manages `aria-controls`, `aria-labelledby`, and `aria-selected` state; and exposes a clean React-style API. *(both agents converge on this encapsulation pattern.)*
...
**The two focus-management strategies for composite widgets** *(both agents — external's framing is sharper, adopting the table here)*:
...
Components that change what's on screen must manage focus deliberately *(both agents)*:
...
- **Toast/notification appears** → does *not* steal focus. Provide a way to navigate to it (`aria-live` for announcement, a hidden landmark, or a key shortcut). *(Claude only — toast-doesn't-steal-focus is a frequent failure mode worth naming explicitly.)*
- **Page navigation** (SPA route change) → manage focus on the new view's main heading or `<main>` element; announce via `aria-live`. *(Claude only.)*
...
- **Error summaries.** *(Claude only — keep.)* For longer forms, an error summary at the top with links to each errored field aids navigation.
...
- **Above-the-input labels** *(Claude only — keep.)* translate cleanly across locales (see 13-internationalization-and-rtl); inline labels (label-on-the-left) break with text expansion.
...
- **Initial reading order** *(external only — useful addition)*. Catchable with annotation tools, before code.
- **200% text reflow simulation** via Text Resizer plugin *(external only)*. Useful for catching layout fragility under user font scaling, with the caveat that browser reflow logic differs.
...
- **Touch-drag gestures** *(external only — keep.)*
...
The **Microsoft A11y Annotation Kit** is a well-developed Figma library of annotation components covering most of these concerns; many teams use it as a starting point and adapt. *(both agents — external lists "stickers"; we use the standard "annotations" terminology.)*
...
For each component shipped, the design spec must include *(both agents)*:
...
**Components that don't ship with this in their spec are incomplete handoffs.** Treat these as definition-of-done, not nice-to-have *(external only — useful framing)*. (See 12-figma-practice on component handoff structure generally.)
...
The accessibility documentation a consumer needs is different from what a component author needs. **Consumers need to know what they can rely on the component to handle and what they must do themselves.** That two-part declaration is the load-bearing piece. *(both agents.)*

Per-component, the docs should cover:

- **Behavioral summary.** *(External only — useful pattern.)* Plain-language description of how the component behaves for assistive technology. "This is a tabbed interface where arrow keys move focus between tabs and Enter activates content."
...
The **three audiences** (designers, developers, QA engineers) *(external only — useful framing)* each need a different cut of this information; the documentation should be navigable per-audience without forcing one to read the others.
...
- **Tooltip vs. helper text.** "Do not use a Tooltip for essential information, as it is not consistently accessible to touch or screen reader users. Use Helper Text instead." *(both agents agree on this; external phrasing kept.)*
- **MenuButton vs. Combobox vs. Select.** "Use a MenuButton when the user is triggering an action. Use a Select or Combobox when they are choosing a value from a list." *(external only.)*
...
Automated tools (axe-core is the dominant engine) catch a real subset of issues but never the whole picture. **Industry figures vary by source — Deque cites figures around 30–50% of WCAG issues being machine-detectable; the external agent cites "approximately 57%" in a more recent framing.** *(External cites 57%; Claude cited 30-50%; both align with Deque's published ranges. Synthesis: cite the source rather than asserting either number; the broader point is that automation is necessary, not sufficient.)*
...
The screen-reader landscape is fragmented; testing all of them is not realistic. The pragmatic minimum *(both agents converge on this matrix; external's table form adopted)*:
...
The **ACR / VPAT** *(external only — adopt this)* is the public, formal version of this promise. Publishing it raises the bar; not publishing it is increasingly read as a signal that the system isn't sure of its own conformance.

### Accessibility debt

Existing component libraries carry accessibility debt — components built before the team had accessibility expertise, components with deferred fixes, components whose patterns weren't audited. **Prioritization matrix** *(external only — useful structure)*:
...
- **If the client insists, document the deviation.** A formal **Risk Assessment** template *(external framing)* or **architecture decision record (ADR)** capturing the request, the specific WCAG criteria violated, the accessibility cost, the legal/regulatory risk, and the named decision-maker.
...
- **Automated checks run; manual testing skipped.** The 30–50% / 57% gap (whichever number) is what ships to production.
...
2. **AI as accessibility gate vs. advisory.** AI can run a first-pass accessibility review on PRs reliably. The decision is whether AI-flagged issues block merge or surface to the human reviewer. As models improve, the balance shifts; today the balance favors advisory. *(Connects to 15 — AI in DS practice on the broader gate-vs-advisory question.)*
...
*Provenance: The four-layer accessibility decomposition (tokens / components / docs / contribution-gate), the pair-tokens contrast architecture (consistent with Adobe Spectrum's model and parallel work in Polaris and Carbon), the WCAG 2.2 AA floor with AAA exceptions documented, the WCAG 2.2 specific criteria (2.4.11 Focus Appearance, 2.5.5 / 2.5.8 Target Size), the focus-management responsibilities (trap, initial, return), the avoid-when patterns (Tooltip for essential info, MenuButton vs. Combobox vs. Select, Toast vs. inline message), the screen-reader testing matrix (NVDA + Firefox/Chrome, VoiceOver + Safari, TalkBack + Chrome, JAWS for regulated), and the deprecation pattern for accessibility debt are convergent practitioner consensus across both research passes. The W3C ARIA Authoring Practices Guide (APG) is the canonical reference for composite-widget keyboard patterns; component compliance is measured against APG. The Accessibility Conformance Report (ACR) and VPAT are the formal public artifacts adopted from Section 508 procurement context. The European Accessibility Act (EAA) became enforceable June 28, 2025; verify against current EU sources before quoting in client materials. APCA and its candidacy for WCAG 3 are documented at github.com/Myndex/SAPC-APCA; standardization status remains provisional. Industry figures cited (axe-core machine-detectable share, retrofit cost multiples, EAA scope) should be attributed to source (Deque, IBM, WebAIM, EU regulatory text) rather than asserted as practice POV. The disabled-vs-aria-disabled debate, the toast-doesn't-steal-focus pattern, the SPA route-change focus management, the form-error-summary pattern, and the cultural-color / accessibility tie-in are positions sharpened from internal practice work and connect to 13-internationalization-and-rtl. The ARIA encapsulation pattern (system absorbs ARIA complexity, exposes high-level props) and "components must implement APG patterns fully" are convergent practice. Stark, Able, Contrast, Microsoft A11y Annotation Kit, and Text Resizer Figma plugins are well-known design-time tools; their specific feature sets evolve. axe-core, Storybook a11y addon, Playwright accessibility, and Pa11y are the dominant CI-side tools. This file was synthesized from a dual-agent research run in May 2026 (see `_research/_archive/2026-05-05-accessibility.md` for the editorial trail).*
