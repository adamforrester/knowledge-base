...
### 1.27 Source-text backing for claims surfaced in 2026-06-15 dual-agent runs — MOSTLY CLOSED

**Closed by the 2026-06-15 evening source-text pass.** Six of the original seven primary-source flags landed; the two watch items both produced their first quarterly check; one primary-source flag (CSS Color 4 §14.2 body text) remains open because the spec exceeds automated-fetch windows and needs a human read. New `_source-text/` files captured: `ext-css-color-4-gamut-mapping.txt` (partial — TOC verified, body text incomplete), `ext-apca-thresholds.txt`, `ext-dtcg-format-module.txt`, `ext-munich-google-fonts-ruling.txt`, `ext-web-almanac-fonts.txt`, `ext-variable-font-axis-byte-costs.txt`, `ext-style-dictionary-changelog.txt`. Inline corrections landed in 31 §1 (DTCG colorSpace enumeration confirmed and de-hedged), 31 §1 gamut-mapping (algorithm structure verified, default-attribution still pending body-text capture), 31 §3 (APCA Lc table tightened against the canonical font-size × weight matrix), 31 §3 (APCA verification path expanded with `readtech.org/ARC/`), 23 §DTCG composite typography tokens (property-level-aliasing-via-`$ref` rule named explicitly), and 23 §Self-hosted vs. third-party hosting (Munich ruling specifics tightened with case number, court chamber, legal basis, narrowness caveat).

**What landed:**
- **§1.27a — MINDE Chroma Reduction algorithm name (PARTIAL).** CSS Color 4's TOC verified through fetch — `Closest Color (MINDE)` and `Binary Search Gamut Mapping with Local MINDE` are named section headings; ΔE_OK is a defined distance metric in §20. Three named algorithms sit as siblings in §14.2 (Binary Search with Local MINDE, EdgeSeeker, Ray Trace). Which is the normative CSS default lives in body text that automated fetch could not retrieve — the spec exceeds WebFetch's window. The *category* (chroma-reduction in OKLCh/Oklab) is supportable. The algorithm-name attribution is captured at TOC level only; needs a human read of `https://drafts.csswg.org/css-color-4/#GMA` to fully close.
...
**What remains open:**
- **§1.27a residual.** The CSS Color 4 §14.2 body-text capture for the default-algorithm attribution. Needs a human read of `https://drafts.csswg.org/css-color-4/#GMA`. Not blocking — 31 §1's hedging is now precise about what's verified (TOC structure, algorithm names, ΔE_OK as a defined metric) and what isn't (which of the three §14.2 algorithms is normatively the CSS default). Promote to a tactical 5-minute task at the next vault session.
