...
### 1.27 Source-text backing for claims surfaced in 2026-06-15 dual-agent runs
Tactical follow-ups across the colour and typography deepen passes. None of these is a full research run; each is a targeted `_source-text/` fetch or a watch item for quarterly review.

**Colour-side follow-ups from the 31 run:**
...
**Typography-side follow-ups from the 23-deepen run** (June 2026):
- **Munich Regional Court 2022 ruling on Google Fonts and GDPR** — Az. 3 O 17493/20 (January 2022) is the load-bearing precedent for the self-hosted-fonts-as-2026-default position in 23 §Font loading and performance. The ruling is real and narrow; the field's "loading remote Google Fonts is illegal under EU privacy guidelines" framing over-states the precedent's scope. Need a primary-source fetch of the ruling and a tracker of subsequent EU case law that may have refined the doctrine before quoting verbatim in workshops or client artefacts.
- **HTTP Archive `wght`-axis utilisation figures** (~60% of variable-font pages, external pass). Verify against the current Web Almanac fonts chapter at almanac.httparchive.org before citing the figure in proposals.
- **Per-axis variable-font byte cost ranges** (`+wdth` adds 30–60%, `+opsz` adds another 30–80% over `wght`-only). Practitioner-observed and load-bearing in 23 §Variable-font byte economics; verify per-typeface via Wakamai Fondue and the foundry's published binaries before committing budget figures to client artefacts.
- One open watch item: **DTCG typography composite-type specification** continues to evolve. Verify against the current spec at tr.designtokens.org/format/ before committing token shapes to the spec's edge cases (property-level aliasing fidelity, multi-mode composite resolution, per-script family overrides).
