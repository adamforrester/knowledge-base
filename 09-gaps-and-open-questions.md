...
**Four claims surfaced in 00e flagged for `_source-text/` backing — all CLOSED by the 2026-06-18 housekeeping pass.** Captured in `_source-text/`:

- **§1.26a — Hupe's two essays — CLOSED.** *Burn Baby Burnout* (29 Sept 2023) and *Your Contribution Model is Doomed to Fail* (23 Sept 2022) both captured at amyhupe.co.uk; full body text of both essays in `_source-text/ext-hupe-burn-baby-burnout.txt` and `_source-text/ext-hupe-contribution-model.txt`. Hupe's "capitalist contradiction of contribution" thesis (per the 2022 essay) is verified verbatim and grounds 00e §8's framing. Hupe's five-reason burnout taxonomy and post-launch-crash framing (per the 2023 essay) corroborate 00e §8's burnout cluster.
- **§1.26b — Sparkbox four-stage maturity model — CLOSED.** Captured at sparkbox.com/foundry/design_system_maturity_model in `_source-text/ext-sparkbox-maturity-model.txt`. Ben Callahan (Sparkbox president) is the canonical author; the four stages (Building Version One / Growing Adoption / Surviving the Teenage Years / Evolving a Healthy Product) match 00e §3 verbatim; the model is current (post still live as of 2026-06-18; cross-references the Design Systems Survey 2020+).
- **§1.26c — The Hot Potato Process — CLOSED.** Captured at danmall.com/posts/hot-potato-process/ in `_source-text/ext-hot-potato-process.txt`. Attribution confirmed: Dan Mall and Brad Frost as joint originators; Mall's post is the canonical citation; Frost's blog tag at bradfrost.com/blog/tag/workflow/ is the secondary source. The pattern's operational guidance (sit together; distributed teams via persistent Zoom; recordings via Voxer/Marco Polo for time-zone gaps; "working together means working together" closing rule) is captured verbatim.
- **§1.26d — Banfield 2024 Knapsack post — CLOSED with verdict: drop the reference.** Captured at richardmbanfield.medium.com/improved-integrated-enterprise-product-infrastructure-with-knapsack-40fac706efa4 (6 June 2024) in `_source-text/ext-banfield-knapsack-2024.txt`. Verdict: post is Knapsack-vs-Storybook product-positioning content (Banfield writes for Knapsack's content-marketing track); the architectural pipeline material is already covered in greater depth by 30-generated-from-data-documentation; the post adds nothing load-bearing. 00e does not reference this post inline; no inline correction needed; the residual closes by *dropping* the reference rather than capturing it as authoritative.
...
- **§1.28a — Cristiano Rastelli at Badoo's Cosmos system — CLOSED with substantive correction.** Captured at didoo.medium.com/measuring-the-impact-of-a-design-system-7f925af090f7 (Jan 2019) in `_source-text/ext-rastelli-badoo-cosmos.txt`. **Substantive correction**: the "source-mix-overtime" *phrase* is not Rastelli's — it appears in the secondary uxdesign.cc summary article rather than Rastelli's original. Rastelli's actual technique is git-log-based change-volume visualisation across application and component-library codebases, with the *correlation-and-frequency-of-change* read as the demonstration of impact. Rastelli is explicit ("when humans are involved, not everything can be measured") that the chart is a demonstration, not a single-number metric. 32 §2.8 inline reference updated to cite the Rastelli post directly with the correct technique-attribution.
- **§1.28b — Marcin Treder UXPin "62 shades of gray" — CLOSED via gating-discovery.** Captured in `_source-text/ext-treder-62-shades.txt`. The original UXPin blog post is not directly recoverable as of mid-2026 (likely retired during UXPin's content reorgs); the figure survives via secondary-source compilations (Smashing Magazine's *Design Systems* eBook; "A Guide to UX Design and Development" book). Attribution to Treder (UXPin CEO ~2016–2017) is consistent across multiple independent sources. 32 §4 inline reference updated to cite via the Smashing eBook compilation pathway rather than asserting a direct UXPin URL the practice can't verify resolves.
- **§1.28c — WebAIM screen-reader survey — CLOSED.** Captured Survey #10 (Dec 2023 – Jan 2024, n=1,539) at webaim.org/projects/screenreadersurvey10/ in `_source-text/ext-webaim-sr-survey.txt`. Primary screen-reader figures: JAWS 40.5% / NVDA 37.7% / VoiceOver 9.7%. Common-use figures: NVDA 65.6% / JAWS 60.5% / VoiceOver 43.9% / Narrator 37.3%. Mobile: VoiceOver 70.6% / TalkBack 34.7%. The five most-common SR+browser combinations cover ~75% of respondents (JAWS+Chrome 24.7%, NVDA+Chrome 21.3%, JAWS+Edge 11.4%, NVDA+Firefox 10.0%, VoiceOver+Safari 7.0%). 32 §2.3 inline reference updated with verified figures and the survey citation.
- **§1.28d — Supernova "AI-ready design systems" positioning — CLOSED.** Captured at supernova.io homepage in `_source-text/ext-supernova-ai-ready.txt`. Supernova is fully repositioned around AI-context-management as of mid-2026 (homepage tagline: "Design & engineering context, ready for AI agents"); ships MCP-endpoint distribution in private beta; distributes per-team / per-workflow / per-agent-type contexts to Cursor / Copilot / Claude Code / Codex. The "AI-ready design systems" framing is real and current. 32 §5's ninth-aspirational-lane (AI-readability validation) is grounded by this positioning. Surfaced a partial correction for 34 §5 inline (Supernova has moved from API-only to MCP-native private-beta).
...
- **§1.29a — axe-core WCAG-coverage methodology — CLOSED with major correction.** Captured at deque.com/automated-accessibility-coverage-report/ and deque.com/blog/automated-testing-study-identifies-57-percent-of-digital-accessibility-issues/ in `_source-text/ext-axe-core-coverage.txt`. **Major correction**: the canonical Deque figure is **57.38% issue-volume coverage** (per Deque's 2021 study of 2,000+ audits / 13,000+ pages / 300,000 issues), not the "30–40%" cited in 34 §4. The "20–30%" / "30–40%" figures circulate widely but measure *WCAG-Success-Criteria coverage* (16 of 50 SC ≈ 32%), which Deque has actively worked to disprove since 2021. 34 §4 inline reference rewritten to distinguish both metrics — WCAG-SC coverage (~32%) vs. issue-volume coverage (57.38%) — and cite both with their definitions. Marcy Sutton's earlier "30%" figure (deque.com 2016 blog post) is a *different* claim about manual-effort-reduction, not coverage; commonly conflated.
- **§1.29b — Southleft Figma Console MCP repository — CLOSED.** Captured at github.com/southleft/figma-console-mcp in `_source-text/ext-southleft-console-mcp.txt`. Repo healthy as of 2026-06-18: 1.8k stars, 201 forks, 72 tags, latest commit 2026-06-05, active branch `feature/desktop-bridge-ui-v0.2.1`. Lead maintainer @tpitre. Documentation site at docs.figma-console-mcp.southleft.com confirms compatibility with Claude Desktop, OpenAI Codex, GitHub Copilot, Cursor, Windsurf. Companion skills repo at github.com/southleft/figma-console-mcp-skills exists as an alternative-installation path using Figma's official MCP server with skills overlay. Bus-factor risk per 34 §2 remains real (third-party single-maintainer) but trajectory looks healthy. Licensing not directly captured (sidebar-only on GitHub); flag as residual to verify per use.
- **§1.29c — Workstation-MCP speed-up empirical validation — STANDING.** Not closeable via housekeeping; requires engagement data. 34 §1's task-and-leverage table cites speed-up multipliers (2–4× / 6–10× / 2–3× / 20–40× / 3–4×) drawn from senior-practitioner observation rather than tracked benchmarks. The first three engagements where the workstation stack is shipped as a productised standardisation should track actual speed-up; the empirically-validated multipliers replace the observation-based ones in the next file revision. Standing flag.
...
- **§1.30a — Spectrum / Carbon "10%-of-first-reseller" payback figure — CLOSED with reframing.** Captured at spectrum.adobe.com/page/color-system/ and carbondesignsystem.com/elements/themes/overview/ in `_source-text/ext-spectrum-carbon-themes.txt`. **The 10% figure is practitioner-extension, not Spectrum- or Carbon-published.** What both systems *do* publish is the architectural pattern that makes the cost-drop possible: Spectrum's per-theme semantic-name resolution (`gray-50` resolves to different RGB values per theme; consumer codes against the semantic name); Carbon's `white` / `g10` / `g90` / `g100` four-theme switching with per-theme CSS packages and preserved semantic roles. 33 §3 inline reference rewritten to cite the architectural pattern (citable) rather than asserting a published 10% figure (not citable). Per-reseller cost is bounded by per-reseller QA work (10–25 hours typical), not by the engine investment itself — that's the architectural claim.
- **§1.30b — The five-reseller engine break-even point — STANDING.** Not closeable via housekeeping; requires engagement data. The first ~3–5 reseller-platform engagements where the practice ships an engine should track actual per-reseller cost against the engine investment to refine the break-even point. Standing flag.
- **§1.30c — Brand-onboarding pricing tiers ($5k / $12k / $25k+) — STANDING.** Not closeable via housekeeping; requires engagement data. The first ~5–10 reseller-platform engagements where the practice ships a tiered onboarding fee should track actual margin against the tier. Standing flag.
...
- **§1.31a — DTCG content-token spec status — CLOSED-as-standing-watch.** Captured at github.com/design-tokens/community-group in `_source-text/ext-dtcg-content-token-status.txt`. As of 2026-06-18: 2025.10 stable spec is current; third-editors-draft (2025-07-21) is the active draft; **no content-token discussion exists** in the repo, the spec, or the Format Module meeting notes. The DTCG charter focuses on the smallest surface area; content-tokens are not on the visible roadmap. The practice's `$extensions.<vendor>` content-token shape per 04 §Content as a system layer is correctly framed as bridge investment. Convert to **standing watch** (analogous to §1.27's Style Dictionary `color-mix()` and DTCG typography composite-type watches): re-check 2026-09-18.
- **§1.31b — CareCentrix 75-hour vs. three-driver hour-model calibration — STANDING.** Not closeable via housekeeping; requires engagement data. The first 5–10 engagements where the practice ships a content-system layer with full Copywriter scope should track actual hours against the three-driver model (component count + content-surface complexity + content-token catalogue depth). Standing flag.
- **§1.31c — Voice-and-tone matrix as agency-cross-engagement standard — STANDING.** Not closeable via housekeeping; requires engagement data. Track adoption across the next ~5 engagements that ship a content-system layer; the bet validates if the matrix lands as default Discovery deliverable across all of them. Standing flag.

### 1.32 CMS platforms — modern composable / headless coverage — CLOSED

**Closed by the 2026-06-18 dual-agent run on modern CMS platforms.** The synthesis landed as a deepen on `10-cms-and-platform-integration.md` (one new framing-altitude H2 inserted between §Central problem and §AEM Architecture Fundamentals; six new platform/concept H2 sections appended after §Drupal but before §DS Delivery Model: NPM Package vs. Embedded; ~7,700 words added; 10 grew from 321 lines / ~5,000 words to 764 lines / ~12,700 words). Salesforce Commerce Cloud Page Designer added to scope alongside the original five named sub-gaps per the user's scoping ask; the New Balance engagement context (per 00b §Active engagement examples) grounds the SFCC treatment.

The new framing-altitude H2 covers the four CMS architectural shapes — traditional (AEM, Drupal) / hybrid (WordPress block themes, Salesforce Page Designer) / headless (Sitecore XM Cloud, Contentful, Sanity, Strapi, Storyblok) / composable (Uniform, Vercel-native, Storyblok composable pivot) — as a decision tree above the existing platform-specific material. The synthesis-ready eleven-row comparison table (platform × architectural shape × DS-integration model × authoring authority × content-storage model) is the artefact the practice carries into scoping conversations.

The six new platform/concept sections cover: headless CMS coverage with per-platform depth on Sitecore XM Cloud (JSS SDK + Edge GraphQL with the `too nested to execute` and `__`-prefix system-field exclusion gotchas; ~600–1,200 hour build scope), Contentful (Vercel Content Link / Stega-encoded source maps as the canonical preview pattern with the `splitEncoding` letter-spacing-breaking gotcha; cleanest CMS-vs-DS boundary in the field), Sanity (Studio customisability with DS components imported into the authoring interface; `vercelStegaSplit` for preview hydration; ~700–1,400 hour build scope), Strapi (Document Service API replacing Entity Service API in v5; v4→v5 migration as breaking change; **v5.47 ships native MCP server**), Storyblok (visual editor with iframe-sandbox tradeoffs and per-component preview-environment hardening); WordPress as DS host (block themes + theme.json mapping to design tokens via Style Dictionary transform; native Block API vs. ACF Blocks tradeoff; DS-vs-block-pattern boundary discipline; headless WordPress with WPGraphQL + Faust.js); Salesforce Commerce Cloud Page Designer (the New Balance / Chakra UI context with Pattern A/B for Chakra-extension-vs-wrapping; SFRA + Page Designer + PWA Kit + Storefront Next stack; React Router 7 + streaming SSR + `initializeRegistry()` registry pattern; the per-component SFCC overhead at 13–45 hours per component; 500–1,400 hour build ranges); the CMS-vs-DS authoring boundary (component-anatomy boundary table extending 04 §Content as a system layer; the `.ai.json` registry sourcing from both DS code and CMS schema with the worked example; three named failure modes — CMS-instance-as-style-override, CMS-as-component-author, DS-as-content-store); MCP / AI-readability for CMS-managed components (per-CMS feasibility table with **major mid-2026 reality**: Contentful, Sanity, Strapi v5.47, WordPress via Automattic Abilities API all shipping official MCP today — significantly faster than 34 §5's bet); composable-content patterns (Uniform / Vercel + V0 / Storyblok composable pivot; the **Edgio (formerly Layer0) Chapter 11 bankruptcy in late 2024** consolidation event; multi-service preview as the hardest UX problem; variability-by-design applied to content; ~1.5–2× headless engagement-scope multiplier).

Both agents converged at high rate (~94%) on every load-bearing claim — the four-axis framing with identical platform-to-shape assignments; the eleven-row comparison table structure; the five-platform headless taxonomy with the same use-case decision drivers; the three failure modes from conflation; the DS-vs-block-pattern boundary discipline; the two SFCC-Chakra patterns; the boundary table content; the `list_content_types` / `get_content_type` / `query_entries` / `get_entry` MCP-tool extension pattern; the CMS-as-DS-source pattern as anti-pattern; the composable engagement scope multiplier. ~94% sits in the band §1.30's ~95% / §1.31's ~97% / §1.29's ~90% / §1.28's ~85% / §1.26's ~70% — fifth-highest convergence the practice has measured.

Two artefacts adopted from the external pass: the eleven-row comparison table format with horizontal architectural-shape grouping (cleaner reading flow than Claude's grouped-by-shape variant); the per-CMS feasibility table at §MCP shaped against the major mid-2026 finding that Contentful / Sanity / Strapi / WordPress all ship native MCP today. Lifted from external specifically: the Sitecore "too nested to execute" GraphQL depth limit + `__`-prefix system-field exclusion patches + Next.js `getStaticProps` workaround; the Vercel Content Link / Content Source Maps / Stega-encoded steganography pattern across Contentful and Sanity; the `splitEncoding` letter-spacing-breaking gotcha and the `vercelStegaSplit` utility; Strapi v5's specific Document Service API / `data.attributes` flattening / `documentId`-vs-`id` / Webpack→Vite changes; Strapi v5.47 native MCP server reality; Sanity's hosted MCP at `mcp.sanity.io`; Contentful's official MCP server reality; WordPress's Automattic Abilities API + `mcp-wordpress-remote` proxy + InstaWP one-click deployment + WooCommerce-specific MCP; the Edgio Chapter 11 bankruptcy late-2024 consolidation event; SFCC Storefront Next + React Router 7 + streaming SSR + `initializeRegistry()` registry pattern. Preserved from Claude's pass: the eleven-row comparison table that splits WordPress-block-themes vs. WordPress-headless as separate rows; the per-component SFCC wrapping overhead breakdown (13–45 hours per component); the four engagement shapes with hour ranges in the SFCC section; the composable engagement breakdown into orchestration integration + preview infrastructure + variant logging sub-deliverables; the worked `.ai.json` example with `structure` + `instances` keys; the references-organised-by-section footer.

Inline cross-references added from 10's new sections to 03 / 04 / 13 / 17 / 24 / 28 / 30 / 33 / 34. Cross-reference back from 34 §5 added inline (mid-2026 MCP shipping reality across both design-token and CMS vendor categories). The 00b SFCC New Balance / Chakra UI context now has a load-bearing destination in 10 §Salesforce Commerce Cloud Page Designer.

**Three claims surfaced in 10's deepen need empirical or temporal validation; logged as §1.32a–§1.32c residuals.** §1.32a converts to a standing watch alongside §1.31a + the §1.27 watches.

- **§1.32a — Four-axis CMS framing evolution — STANDING WATCH.** The four-axis framing (traditional / hybrid / headless / composable) represents the stable mid-2026 consensus per Forrester / Gartner / Real Story Group. The practice should track quarterly for emergence of a definitive fifth shape — the "Agentic / AI-Native CMS" — where content is dynamically generated rather than authored, fundamentally altering the schema contract. Track quarterly against emerging features in platforms like Uniform's Agentic DXP direction and Anthropic-integrated workflows. Re-check 2026-09-18.
- **§1.32b — Per-CMS MCP-wrapping hours empirical validation — STANDING.** The 16–60-hour range across CMS platforms is observation-grounded against the platforms' API-shape complexity. The first 3–5 engagements per CMS where the practice ships an MCP wrapper should track actual hours; the empirically-validated wrapping-hour ranges replace the observation-based ones in the next 10 revision. Standing flag.
- **§1.32c — SFCC per-component wrapping overhead empirical validation — STANDING.** The 13–45-hour-per-component range for Page Designer wrapping (component definition + configuration schema + render template + region metadata) is observation-grounded against historical SFCC engagements. The next 3–5 SFCC engagements should track actual per-component overhead; the validated range replaces the observation-based one in the next 10 revision. Standing flag.
...
- **A worked example now exists.** PJ Onori's DSDS governs its own schema with exactly this discipline — single source of truth for the version number, patch = new optional fields / minor-major = breaking, scripted migrations per bump, AJV validation in CI, and an editorial lint that warns on quality gaps without failing the build. It also separates structural rules (enforced in the schema) from quality rules (a catalogue with stable IDs), following Adobe's Spectrum Design Data. Worth reading before designing ours (see 29 §6).
- **Status:** open. Tracked as a work item in the issue queue.

### 1.36 Marketing-page blocks — the composition genre neither layer covers

- Surfaced from the Prism3 build (field note at `13-inspirations` §8 in the prism3 repo, reviewed 2026-08-10): the practice has **no position on section-level marketing compositions** — hero, pricing table, testimonial band, logo wall, bento grid, CTA, stats strip — the things commercial libraries call **blocks**.
- **Why neither existing layer holds it.** `components/` studies individual things. `patterns/` studies compositions, but is keyed to a **user goal** (`ask-for-data`, `navigate`, `show-data`, `help-user-decide`, `recover-from-error`), and `patterns/app-shell.md` states outright that it does *not* apply to a marketing site. **A hero section resolves no user goal.** Blocks are keyed to **page role** and to persuasion — a genre the goal facet cannot express, which is the actual gap rather than a filing inconvenience.
- **The field ships this at scale, and the sizes converge.** Initium: 197 blocks across 23 types, plus page templates assembled from them. daisyUI: 211 page architectures matched by intent. Cloudscape: 181 addressable exemplars at predictable URLs. Two hundred-ish is apparently the shape of a complete marketing-composition library, and the depth is steeply uneven — hero and feature families run 20+ layouts where team and career run 3–4.
- **The interesting question is not the catalogue, it is the axes.** Twenty-two hero layouts is a handful of axes multiplied out — media placement, content alignment, container treatment, action count, social-proof slot — pruned to the combinations worth shipping. A practice POV that lists blocks is a shopping list; one that names the **axes per block family, and the pruning rationale**, is reusable across engagements and is what an engagement's own block library should be generated from. This is the composition-tier analogue of the generative-token position in 24.
- **One accessibility contract makes this urgent rather than academic.** `components/section.md` already carries the don't-over-landmark rule — twelve benefit blocks must be *one* named `<section>` over a grid of twelve generic containers, not twelve landmarks. **A page assembled by picking blocks produces the twelve-landmark page by construction**, and the failure is invisible to everyone not using AT. Any block POV the practice publishes has to own page-level landmark structure, or it ships the defect the component brief already warns about.
- **The commercial libraries are marketing-shaped, and the practice's most common surface is not in them.** All 23 types in the reference library are SaaS-marketing blocks — no product grid, no faceted filtering, no product detail page, no cart. **A block POV built by mirroring the field would miss commerce silently**, since nothing in a type list announces an omission. PDPs are bespoke per brand and genuinely vary, but they share a skeleton — media gallery, title/price/rating, variant selection, quantity and add-to-cart, delivery promise, specification accordions, reviews, recommendations, sticky mobile buy bar — and *high variation over a shared skeleton* is exactly the condition that makes a family worth deriving rather than hand-building per engagement. For an agency whose work is weighted to commerce (00b), this is the half of the taxonomy worth having first, not the half to append.
