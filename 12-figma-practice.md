...
Two newer capabilities are worth watching: **composite/array types** (grouping shadow x/y/blur/spread/color into a single variable rather than five separate ones) and **expression tokens** (programmable conditional logic inside a variable, e.g. `if(is-dark, #FFF, #111)`). These have been variously dated to 2025–2026 — composite types as a 2025 update, expression tokens as a 2026 preview — but we have not independently verified them as shipped features. If they are stable in current Figma, they significantly reduce the gap to Tokens Studio. **Treat as "verify before recommending"** until our own pilot confirms the feature names, scope, and stability. This is a sensible thing to flag during Discovery as something to watch over the engagement window.
