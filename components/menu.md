...
- **Roving tabindex, not a focus trap.** Restore focus to the trigger on close (Escape/select/outside-click); close on Tab. Misapplying a Dialog-style trap (see dialog) prevents power users from tabbing through a form that contains menu triggers.
...
- **Stands on:** Button (the trigger — see button), the overlay/positioning substrate (collision/flip, portal, dismiss — established by select and combobox, now formalised in popover — see popover), Icon (leading icons, kebab, submenu chevron — see icon), Divider (the `role=separator` — see divider).
...
api:
  inherits:
    ...
    - "overlay-positioning-substrate (collision/flip, portal, Popover API + CSS Anchor Positioning, dismiss — established by select/combobox; now formalised in popover)"
...
composition:
  stands-on: [button, overlay-positioning-substrate (select/combobox; formalised in popover), icon, divider]
...
notes:
  ...
  inherited: "trigger from button; the entire overlay/positioning substrate from select/combobox (formalised in popover)"
