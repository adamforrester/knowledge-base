...
- **Trailing badges/counts** are subordinate, far-edge, concise ("3", "99+", "New") with an accessible name ("3 unread") — the count belongs in the nav item's accessible name, not a floating number (see badge for the announcement contract); **hidden in the rail** (they'd collide with the icon).
...
- **The persistent layout shell** — the side nav is a **column in the app's grid/flex shell** (`grid-template-columns: var(--sidebar-width) 1fr`), not a `position:fixed` overlay; collapsing animates the column width + reflows content. The shell *around* this column — the header zones, the top-vs-side-vs-hybrid placement decision, the responsive collapse to a drawer/bottom-nav, the landmark/skip-link skeleton, and the SPA route-change contract — is the patterns layer's job (see patterns/app-shell); this component owns the nav control, that pattern owns the frame that places it.
...
- **Alternative to / boundary with:** the **top nav / app header** (shallow/marketing or global-utilities-only vs deep-IA persistent side nav — not yet briefed), **Tabs** (in-page panel switching, not cross-page nav — see tabs), **Breadcrumbs** (a secondary location trail — see breadcrumbs), **Drawer** (transient overlay; the side nav *is* a drawer on mobile — see drawer), **Menu** (`role=menu` commands, not nav links — see menu; the nav-is-not-a-menu boundary), **Tree** (`role=tree` for hierarchical *data*, not site nav — the boundary drawn from both sides; see tree).
