...
What it *isn't*: a **Progress bar** (a measurable/longer task with a value — see progress; the semantic contract differs entirely — progress implies eventual completion, a spinner implies busyness), or a **Skeleton** (a content-shaped placeholder for an *initial* load — see skeleton when briefed; a spinner is structurally agnostic and gives no shape). Why systems disagree: separate-primitive-vs-variant, the announcement model, and inline-vs-overlay placement.
