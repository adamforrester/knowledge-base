...
- **Decision frameworks:**
  ...
  - **Actionable toasts shouldn't auto-dismiss** — if there must be an Undo, give it `Infinity` (or a generous, paused timeout) and a real keyboard route (§6); racing a timer to click Undo is the anti-pattern. If the action matters, it belongs in a persistent Banner. The undo *model* this Undo serves — optimistic execution, the soft-delete grace period, when to undo vs confirm — is the patterns layer's (see patterns/destructive-confirmation); this component owns the surface, that pattern owns the model.
