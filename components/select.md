...
- **Alternative to:** Combobox (searchable/typeahead — the §1 boundary), Radio Group / Segmented Control / Connected Button Group (small sets exposed for one-tap), Checkbox Group (multi-select of a few items), Menu (commands, not value-setting — the select-vs-menu line: a menu *performs an action*, a select *sets a form value*), Switch/Checkbox (boolean).
