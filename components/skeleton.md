...
- **Composed by / hosts:** the **Data / Card / List / Image** components whose initial-load state is a skeleton (Table renders skeleton rows, a Card a skeleton, an Image a skeleton box) — the skeleton must import the host's radius/padding/aspect-ratio tokens for a 1:1 match (see table/list/card/image when briefed).
