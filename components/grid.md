...
- **Boundary (the naming collision):** the **interactive ARIA data grid** (`role="grid"`) / **Table** — a tabular widget with cell navigation and explicit semantic roles (the pattern Tag's removable-token group borrows; the role=grid boundary lands in table). A layout Grid is *not* that and must not claim its role (§6).
...
composition:
  ...
  boundary: ["interactive ARIA data grid (role=grid)", table]   # the naming collision; a layout Grid is NOT a data grid (the role=grid boundary lands in table)
