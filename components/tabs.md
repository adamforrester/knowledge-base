...
- **Panel focus** stays on the tab after activation (the user keeps arrowing); the panel is reachable via Tab. Count badges announce coherently ("Errors, 4 items", via visually-hidden text in the tab's accessible name, not a floating number — see badge for the announcement contract), and icon-only tabs need an `aria-label` + tooltip.
