...
- **The year-navigation problem (the birthdate trap):** a calendar defaulting to the current month is hostile for a birthdate 40 years ago (40 month-clicks backward — a catastrophic usability failure). For far-from-today dates the **segmented input is the right primary** (type "1985"), and the calendar needs a **fast year/month jump** (a year dropdown/grid), not just month chevrons. For *date of birth specifically*, the patterns layer goes further: the field-truth answer is **not this component at all** but three plain text inputs (day/month/year) — see patterns/data-collection-recipes, which holds the DOB-refuses-the-picker boundary.
