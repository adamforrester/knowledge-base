...
- **Don't use a Badge for:**
  - An **interactive/removable token** → a **Tag/Chip** (filter chips, removable selections). The boundary: a Badge has no `onClick`, no `×`, no selected state. (Tag not yet briefed; this brief draws the boundary.)
...
- **The Badge-vs-Tag disambiguation (the load-bearing naming call):** **Badge = non-interactive, non-removable indicator**; **Tag/Chip = interactive/removable/selectable token**. If it has an `onClick`, an `×`/dismiss, or a selected state, it is a **Tag**, regardless of what a vendor library calls it. **"Pill"** describes a `border-radius` token, **not** a component — relegate it. (Tag not yet briefed; this is the boundary it will inherit.)
...
- **Boundary:** **Tag/Chip** — the interactive/removable/selectable token (Badge is the passive indicator; the strictest functional boundary in the system; Tag not yet briefed).
