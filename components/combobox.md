...
- **Decision frameworks:**
  ...
  - **Combobox vs. Predictive Search is the output** — a Combobox *sets a form value*; predictive search (Hydrogen) *emits a link and routes away*. Never embed predictive-search-routing in a data-entry form, and never use a value-setting combobox for global navigation (external). When the combobox *is* the input to a search experience (suggestions, results, no-results, scope), the orchestration around it is the patterns layer's job — see patterns/search-orchestration; this brief owns the control, that pattern owns the experience.
...
- **Async race conditions** — debounce input (~300ms, per Hydrogen's predictive search) and **cancel stale requests with `AbortController`** (or a request-id guard) so a slow earlier query can't overwrite a fast later one. Surface min-query / input-loading / pagination-loading / no-results / error as distinct states. (This `AbortController` cancel/race model recurs across the catalogue's async components — File Upload reuses it for per-file upload cancel/retry; see file-upload.)
