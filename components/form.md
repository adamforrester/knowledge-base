...
- **The pattern (out of scope — now briefed):** **Forms-as-a-system** — the multi-field form as a *system* (validation timing, the error-summary + focus contract, conditional reveal, cross-field rules, the save model, the threshold). Named here as the boundary; resolved in the patterns layer (see patterns/forms-as-a-system). Note the split the pattern draws: *Forms-as-a-system* is the single-view orchestration; the multi-step **wizard** is its sibling `flow` pattern, and the long multi-section flow / save-and-resume graduates into it. (Polaris and Carbon draw the same line: a `Form` component for single-view orchestration, multi-step delegated to pattern.)
...
- **When to reach for the wizard:** a long form (disparate sections — personal then payment — or a soft ~10–15-field band, or one needing external documents) graduates out of a single-view form. *Forms-as-a-system* owns the single-view orchestration and the threshold decision (see patterns/forms-as-a-system); above it sits the **multi-step wizard** (its sibling `flow` pattern), which owns the progression, draft-saving, and cross-step validation. The Form component still governs each *step*.
...
- **Borders (the component-vs-pattern boundary):** the **Forms-as-a-system pattern** (see patterns/forms-as-a-system) — the multi-field form as a system, *not* this component; it graduates into the **multi-step wizard** (its sibling `flow` pattern — routing + global state) for the long multi-section flow and save-and-resume, while the Form governs each step; the **Filter set** (fields applied without a submit — the Filtering pattern).
