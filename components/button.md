...
- **Layout container** — an `inline-flex` row that centres content and owns the gap between the leading slot, label, and trailing slot. Note where that gap *comes from*: in the token-mature systems it is not a button-local value but a **shared system scale** the button merely references — Spectrum resolves it from `text-to-visual-{75,100,200,300}` and sizes the visual from `workflow-icon-size-*`, Material from `icon-label-space` and `icon-size`, Carbon from the general `$spacing-*` ramp (Spectrum CSS `button/index.css`; Material Web `_md-comp-filled-button.scss`; Carbon `_button.scss` — all 2026). The label↔visual gap and the icon size are foundations concerns, not component concerns; a system that discovers them at the component tier has usually mis-scoped them.
- **Horizontal padding is slot-aware, and deliberately asymmetric** — the padding on the side where a *visual* sits is smaller than on the label side. The reason is optical, not arbitrary: an icon's own bounding box contributes apparent space, so equal numeric padding reads as *too much* on the icon side. Three systems encode this independently, which makes it a convergence rather than a house style: Material ships `leading-space: 24px` against `with-leading-icon-leading-space: 16px` (and mirrors it for a trailing icon); Spectrum separates `edge-to-text` from `edge-to-visual` as distinct token scales entirely; Carbon trims the icon side of its ghost variants by a converted pixel (same sources, 2026). A single `padding-x` per size is the simplification most systems ship first and then outgrow — worth pricing in at foundations rather than retrofitting, since splitting it later touches every binding downstream.
- **Some dimensions are derived, not authored** — Spectrum computes minimum width as `height × minimum-width-multiplier` and the pill radius as `height / 2` rather than tokenising either (Spectrum CSS `button/index.css`, 2026). Worth naming as its own category: a spec that treats every dimension as an authored token will either over-tokenise or hard-code the relationships it should be expressing.
...
The full decision space runs from one overloaded enum, through the two-axis intent + appearance model we adopt, to a maximally-decoupled **three-axis split** (`emphasis` × `tone` × `fill`) that buys the clean low-emphasis-destructive case at the cost of the largest visual matrix and the most token-theming work. We sit deliberately in the middle — enough separation to kill enum bloat, not so much that the matrix and theming cost explode.

**Prism3 adoption (downstream reconciliation, 2026-07-06).** The practice's generation engine fixes this model to a settled vocabulary; the field study above is unchanged, and the reconciliation is recorded where it is authoritative — in the implementation, not the brief. Appearances become `filled` / `outline` / `text` (for `solid` / `outline` / `plain` — no room for interpretation on "plain" or "ghost"); colours become `primary` / `neutral` / `destructive` plus an *optional* `accent`. `ghost` is retired — it was an appearance masquerading as a colour; `neutral` is promoted to a first-class colour rather than a borrowed surface; and `accent` is optional, generated only when a brand ships a second colour and never faked from primary. The reasoning is §3's own logic carried through: emphasis is the *appearance* axis's job, so there is no `secondary` rung; the field-rare accent stays optional; and `neutral` — the hardest colour to resolve across modes (§6, and the practice's own long-standing miss) — earns first-class, generated, contrast-verified treatment rather than the hand-tuned approximations that never quite held. The full interactive-colour taxonomy, the mechanism that finally makes neutral tractable, and the boundary between interaction states and non-interactive layers (scrims) live in the Prism3 engine's decision record, `Prism3/docs/20-interactive-color-system`. Per the brief-upstream / engagement-data-downstream relationship (see 30), that record is prescriptive for the build; this brief remains the field record.
...
- **Alternative to:** Link (navigation vs. action — the core boundary; see link, which closes it from the link side), IconButton (icon-only specialisation), LinkButton (navigation that looks like a button), ToggleButton (`aria-pressed` persistent state — see toggle-button), SplitButton (default action + adjacent menu of related actions — the primary action is never duplicated inside the menu; Fluent 2, 2024), Switch (binary setting), Chip/Tag (removable/selectable token).
