...
- **The upload sibling:** an editable avatar is the single-file image-upload-with-crop affordance — a **File Upload** in its avatar/replace layout (the preview replaces the drop zone, with hover Edit/Delete), not the multi-file list (see file-upload).
