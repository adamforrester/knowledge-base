...
- **Substrate FOR (the inheritors):** **Stack** (1D flex, owns `gap`), **Inline** (horizontal flex), **Grid**/**Columns** (2D, tracks/areas/`gap`), **Card** (a Box pre-binding background/radius/padding tokens), **Section** (a semantic region) — all built on Box, all inheriting the spacing-token scale + the `asChild` polymorphism (all briefed — see stack/grid/card/section; this brief is their substrate, and with Section the Layout category is complete).
