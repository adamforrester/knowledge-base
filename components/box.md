...
- **Substrate FOR (the inheritors):** **Stack** (1D flex, owns `gap`), **Inline** (horizontal flex), **Grid**/**Columns** (2D, tracks/areas/`gap`), **Card** (a Box pre-binding background/radius/padding tokens), **Section** (a semantic region) — all built on Box, all inheriting the spacing-token scale + the `asChild` polymorphism (Stack/Grid/Card/Section not yet briefed; this brief is their substrate).
...
api:
  ...
  substrate-for:
    ...
    - card           # a Box pre-binding bg/radius/padding tokens (not yet briefed)
