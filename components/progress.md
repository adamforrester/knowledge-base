...
- **Spinner** — the minimal *indeterminate inline busy* indicator (~1–5s; a button, a dropdown fetching) — no value contract — see spinner when briefed.
...
- **Alternative to / boundary with:** **Spinner** (minimal indeterminate inline busy indicator, no value contract — see spinner when briefed), **Skeleton** (content-shaped initial-load placeholder, prevents CLS — see skeleton when briefed), **Meter** (`role=meter` static measurement — a separate component), **Stepper** (discrete *user* steps vs system labour), the **loading/promise Toast** (see toast §promise — shades into progress feedback).
