...
- **Is the substrate for (specialisations sitting on Popover):** Menu (`role=menu` content — see menu), the Select listbox (`role=listbox` — see select), the Combobox listbox (see combobox), Tooltip (`role=tooltip`, hint — see tooltip). These inherit Popover's anchored-surface + positioning machinery and add their content role + focus model.
