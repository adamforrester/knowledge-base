...
Per-reseller cost *with* the engine drops dramatically — practitioner observation puts the per-reseller onboarding work at **10–25 hours** (the brand-input meeting, the engine invocation, the QA pass, the handoff) — bounded by the per-reseller QA work rather than the engine investment. The widely-cited "10% of first reseller's cost" figure is practitioner-extension, not Spectrum- or Carbon-published. What Adobe Spectrum and IBM Carbon *do* publish is the architectural pattern that makes the cost-drop possible: Spectrum's per-theme semantic-name resolution (`gray-50` resolves to `rgb(255,255,255)` in light theme, dark in dark theme; the consumer codes against the semantic name and the engine handles per-theme resolution); Carbon's `white` / `g10` / `g90` / `g100` theme-swap pattern with per-theme CSS packages and the `$ui-background` semantic role preserved across themes. The architecture is the citable pattern; the 10% figure is the practitioner-extension claim. Source: `_source-text/ext-spectrum-carbon-themes.txt`.
